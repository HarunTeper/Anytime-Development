...
AnytimeActionServer::AnytimeActionServer(const rclcpp::NodeOptions & options) : Node("anytime_action_server", options), model(create_config())
{
    ...
    timer_ = this->create_wall_timer(std::chrono::milliseconds(0),[this](){execute();}, callback_group1_);
    timer_->cancel();
...
void AnytimeActionServer::execute(const std::shared_ptr<AnytimeGoalHandle> goal_handle, std::shared_ptr<rclcpp::TimerBase> timer){
    (void)timer;
    if (this->forward_one()){
        end_time = this->now();
