...
YOLOModel::YOLOModel(YOLOModelConfig config) : stream(config.stream) {
  std::cout << "Loading model from " << config.weights_path << std::endl;
  std::cout << config.weights_path << std::endl;
  std::cout << "/home/f110xaviernx/ros2_rt_yolo/src/yolo_rt/weights/"
               "yolov7-tiny-traced-half.pt"
            << std::endl;
...
torch::IValue layer_forward(Layer& layer, std::vector<torch::IValue> outputs,
                            torch::IValue previous_output) {
  auto source_layer_info = layer.source_info;
  torch::IValue output;
  std::visit(
      [&previous_output, &outputs, &output, &layer](auto&& arg) {
        using T = std::decay_t<decltype(arg)>;
        if constexpr (std::is_same_v<T, int>) {
          torch::IValue input;
          if (arg == -1) {
...
torch::IValue YOLOModel::forward_full(torch::Tensor x) {
  ...
  int i = 0;
  // std::cout << "Running model with " << this->layers.size() << " layers" <<
  // std::endl;
  for (auto& layer : this->layers) {
    // std::cout << "Running layer " << i << " with source info " <<
    // layer_src_info_to_string(layer.source_info) << std::endl;
...
bool YOLOModel::forward_one() {
  if (this->stream.query()) {
    ...
    // std::cout << "Running layer " << layer.id << " with source info " <<
    // layer_src_info_to_string(layer.source_info) << std::endl;
    auto output =
