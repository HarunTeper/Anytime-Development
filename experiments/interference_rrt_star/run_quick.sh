#!/bin/bash
# Interference RRT* Quick Experiments — shortened version for AE evaluation
# Demonstrates the same trends as the full run with fewer configurations
# Usage: ./experiments/interference_rrt_star/run_quick.sh
...
echo "========================================="
echo "Interference RRT* Quick Experiments"
echo "========================================="
echo ""
    ...
    for mode in "${MODES[@]}"; do
        for thread_mode in "${THREADING[@]}"; do
            for run in $(seq 1 ${NUM_RUNS}); do
                ...
                ros2 launch experiments interference_rrt_star.launch.py \
                    server_config:="${server_config}" \
                    client_config:="${client_config}" \
                    interference_config:="${CONFIG_DIR}/${config_name}_interference.yaml" \
