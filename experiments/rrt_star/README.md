...
Evaluate RRT* path planning as an anytime workload under the Anytime framework, measuring block-size scaling, threading impact, and path cost convergence on Nav2 occupancy grid maps.
...
| `run_quick.sh` | Quick subset run for development/CI (depot, single-threaded, 3 block sizes) |
...
- **Block sizes**: 1, 16, 64, 256, 1024, 4096
...
```bash
# Default: block_size=256, reactive, single-threaded, depot, 10s
./experiments/rrt_star/test_single_config.sh

# Custom parameters: block_size mode threading map duration
...
### Overhead Metrics
- **Per-block overhead** — Gap between consecutive compute blocks (ms)
...
- **Block time percentiles** — p50, p95, p99 of block compute times
...
- `block_size_vs_time[_<map>].pdf` — Block compute time by configuration
...
- `per_block_overhead[_<map>].pdf` — Inter-block overhead
...
- `block_time_percentiles[_<map>].pdf` — p50/p95/p99 distributions
- `block_time_trend[_<map>].pdf` — Block time over run duration
...
- `best_cost_vs_block_size[_<map>].pdf` — Final cost quality
...
│   ├── block_1_reactive_single_depot_server.yaml
│   ├── block_1_reactive_single_depot_client.yaml
│   └── ...
├── traces/                         # LTTng trace data (per run)
│   ├── block_256_reactive_single_depot_run1/
│   └── ...
└── results/
...
| Tracepoint | Purpose |
|---|---|
| `anytime:anytime_compute_entry/exit` | Block computation boundaries |
