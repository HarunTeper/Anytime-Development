...
The experiment measures the **Anytime framework's** scheduling overhead, cancellation responsiveness, and feedback throughput using RRT* path planning as the domain algorithm. The independent variables are block size, cancellation mode (reactive vs proactive), threading model (single vs multi), and map environment. The dependent variables are block compute time, overhead ratio, cancellation latency, and solution quality.
...
|----------|--------|-------|-----------|
| Block size | 1, 16, 64, 256, 1024, 4096 | 6 | Spans single-iteration (isolates per-iteration framework overhead) to large blocks (approaches continuous computation). Roughly powers-of-4 progression. |
...
|-----------|-------|-----------|
| Run duration | 10 s | Long enough for ~20 goal cycles at 500 ms period, producing hundreds to thousands of block measurements per run. |
...
- **Server config** (`block_X_mode_threading_map_server.yaml`): Sets `is_reactive_proactive`, `multi_threading`, `block_size`, `random_seed`, map path (placeholder `MAPS_DIR`), start/goal coordinates, and all RRT* algorithm parameters.
- **Client config** (`block_X_mode_threading_map_client.yaml`): Sets `goal_timer_period_ms` and `cancel_timeout_period_ms`.
...
For each (block_size, mode, threading, map, run_number):

#### Step 1: LTTng Session Setup

- Create a named LTTng session (`rrt_star_exp`) with output to `traces/block_X_mode_threading_map_runN/`.
...
- `anytime_compute_entry` / `anytime_compute_exit` — block computation boundaries
...
This difference is fundamental: in multi-threaded mode, a cancel request can be processed on a different thread while compute is running (the compute loop checks `goal_handle_->is_canceling()` between iterations). In single-threaded mode, the cancel callback can only run after the current compute block completes.
...
The key difference: in proactive mode, `calculate_result()` runs every iteration (between compute and feedback), so the result is always ready. In reactive mode, it runs only at termination, reducing per-block overhead but delivering a potentially stale result.

### 4.3 Compute Block

Each `compute()` call executes `block_size` iterations of `compute_single_iteration()`:
...
**Cancellation is per-iteration, not per-block.** Even with block_size=4096, the cancel check runs before every single RRT* iteration. Cancel response delay is bounded by one iteration's time (microseconds to low milliseconds), regardless of block size. The `block_size` parameter controls **overhead-to-compute ratio** and **feedback frequency**, not cancellation responsiveness.

**Missing exit trace on cancellation:** When cancellation occurs mid-block, the `return` at the check skips `TRACE_ANYTIME_COMPUTE_EXIT`. This creates an unmatched `compute_entry` event. The evaluation script handles this by overwriting `current_compute_start` on the next entry — the interrupted block produces no `block_time` measurement. This affects the last block of every goal cycle equally across all configurations.
...
Each run produces a CTF trace directory: `traces/block_X_mode_threading_map_runN/`. These are read by `babeltrace` during evaluation.

### 5.3 What the Traces Capture

| Measurement | How It's Computed | Events Used |
|-------------|-------------------|-------------|
| Block compute time | `compute_exit.timestamp - compute_entry.timestamp` | compute_entry, compute_exit |
| Inter-block overhead | `compute_entry[N+1].timestamp - compute_exit[N].timestamp` (excluding inter-cycle gaps) | compute_entry, compute_exit, rrt_star_reset, base_deactivate |
| Overhead ratio | `overhead / (overhead + block_time) * 100%` | compute_entry, compute_exit |
...
1. Discover all trace directories matching `traces/block_*`.
...
- **Block times**: Paired compute_entry/exit timestamps (ms).
- **Per-block overheads**: Gap between consecutive compute_exit and compute_entry (ms). Gaps that span a goal cycle boundary (detected via `rrt_star_reset` or `base_deactivate` events) are excluded — these represent inter-goal idle time (~300 ms), not framework overhead.
- **Overhead ratios**: Computed at each compute_entry using the previous block's time as the denominator: `overhead / (overhead + prev_block_time) * 100`. Only measured for within-cycle gaps.
...
- **Final metrics (best cost, tree size, total iterations)**: Only the *last* `rrt_star_result` event per goal cycle is used. In proactive mode, `rrt_star_result` fires every block (via `calculate_result()`); using only the last one per cycle avoids inflating `total_iterations` (which would otherwise sum intermediate iteration counts) and prevents early `inf` costs from poisoning `avg_final_best_cost`.

Summary statistics per run: mean, std, percentiles (p50, p95, p99) for block times; mean and std for all other metrics.
...
- **Reactive vs proactive**: For each (block_size, map, threading) triple, pooling raw block_times across runs for each mode.
- **Single vs multi threading**: For each (block_size, map, mode) triple, pooling raw block_times for each threading model.

Mann-Whitney U is chosen over the t-test because block time distributions may be non-normal (especially at small block sizes where OS scheduling noise dominates). The minimum sample size is 5 per group. Results are saved to `results/statistical_tests.csv` with U statistic, p-value, and a significance flag at alpha=0.05.
...
1. Groups all goal cycles by configuration (block_size, mode, threading) across all 5 runs (~100 cycles per config).
...
- `block_size_vs_time[_{map}].pdf` — Block compute time vs block size, grouped by mode x threading
- `throughput[_{map}].pdf` — Iterations per second vs block size
...
- `per_block_overhead[_{map}].pdf` — Inter-block gap time
...
- `block_time_p{50,95,99}[_{map}].pdf` — Block time percentiles, grouped by mode x threading
- `block_time_trend[_{map}].pdf` — Block time over sequential blocks (aggregated by config, mean with std shading)
...
- `best_cost_vs_block_size[_{map}].pdf` — Solution quality at cancellation
...
### What Block Size Controls

Block size does **not** control cancellation responsiveness (which is per-iteration). It controls:

- **Overhead-to-compute ratio**: At block_size=1, the framework overhead path (feedback send, timer checks, waitable rescheduling) runs after every single iteration. At block_size=4096, it runs once per 4096 iterations. The overhead ratio should decrease with larger block sizes.
- **Feedback frequency**: The client receives feedback (current best cost, tree size) once per block. Smaller blocks = more frequent feedback.
- **Convergence tracepoint resolution**: With convergence_log_interval=100, a block of 1 emits a tracepoint every 100 blocks, while a block of 4096 emits one every ~0.024 blocks.

### What the Convergence Plots Show

Not asymptotic RRT* convergence. Each goal cycle gets ~200 ms of compute. The plots show how quickly cost decreases within these short planning windows. Different block sizes may show different convergence depths because the overhead ratio affects how many iterations fit into the 200 ms window.
...
1. **Missing `TRACE_ANYTIME_COMPUTE_EXIT` on mid-block cancellation.** The interrupted block produces no timing measurement. `total_blocks` (incremented at entry) can exceed `len(block_times)` (recorded at exit). Affects the last block of every goal cycle equally across configurations.

2. **Block time non-stationarity.** `find_nearest()` and `find_near()` are O(n) linear scans. Per-iteration cost grows as the tree grows within each cycle. `avg_time_per_block` mixes framework overhead variability with algorithmic complexity growth. The block time trend plots expose this; for pure framework analysis, restrict to early blocks or normalize by tree size.
...
4. **Convergence subsampling.** The `rrt_star_iteration` tracepoint fires every 100 iterations. At large block sizes, this means very few tracepoints per block, giving coarse convergence resolution. At block_size=4096, a single block contains ~41 tracepoint emissions (4096/100), but if a goal cycle only lasts a few blocks, total data points per cycle are sparse.
